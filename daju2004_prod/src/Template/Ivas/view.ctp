...
<div class="w3-container">
    <h3><?= h($iva->label) ?>
        <?php $my_url_edit=$this->Url->build(['controller' => 'ivas','action' => 'edit',$iva->id])  ?>
    ...
    <table class="w3-table w3-border w3-bordered w3-hoverable w3-theme-d4">
        <tr>
            ...
            <td class="w3-border"><?= h($iva->name) ?></td>
        </tr>
        <tr>
            <td class="w3-border"><?= __('Observaciones') ?></td>
            <td class="w3-border"><?= h($iva->observaciones) ?></td>
        </tr>
        <tr>
            <td class="w3-border"><?= __('Id') ?></td>
            <td class="w3-border"><?= $this->Number->format($iva->id) ?></td>
        </tr>
        <tr>
            <td class="w3-border"><?= __('Valor') ?></td>
            <td class="w3-border"><?= $this->Number->format($iva->valor) ?></td>
        </tr>
    </table>
...
<div style="display:none" id="material" class="related w3-container w3-theme-d3 w3-border">
    <h4><?= __('Related Material') ?>
        <?php $my_url_add=$this->Url->build(['controller' => 'material', 'action' => 'add',$iva->id,'iva_id'])?>
...
<?php foreach ($material as $material): ?>
            	...
            	<tr onClick="location.href='<?= $my_url ?>'" class="w3-hover-black ">
                	...
                	<td class="w3-border"><?= h($material->moneda_id) ?></td>
...
<div style="display:none" id="proceso" class="related w3-container w3-theme-d3 w3-border">
    <h4><?= __('Related Proceso') ?>
        <?php $my_url_add=$this->Url->build(['controller' => 'proceso', 'action' => 'add',$iva->id,'iva_id'])?>
        <button onClick="location.href='<?= $my_url_add?>'" class="w3-button w3-xlarge w3-circle w3-black">+</button>
    </h4>
...
<div style="display:none" id="producto" class="related w3-container w3-theme-d3 w3-border">
    <h4><?= __('Related Producto') ?>
        <?php $my_url_add=$this->Url->build(['controller' => 'producto', 'action' => 'add',$iva->id,'iva_id'])?>
        <button onClick="location.href='<?= $my_url_add?>'" class="w3-button w3-xlarge w3-circle w3-black">+</button>
    </h4>
    ...
    <div class="w3-responsive">
		<table class=" w3-table w3-centered w3-border w3-bordered w3-hoverable w3-theme-d4">
        	<thead class="w3-border w3-black">  
                ...
                <th  class="w3-border" scope="col"><?= $this->Paginator->sort('cantidad','cantidad', ['model'=>'Producto']) ?></th>
...
<?php foreach ($producto as $producto): ?>
            	...
            	<tr onClick="location.href='<?= $my_url ?>'" class="w3-hover-black ">
                	...
                	<td class="w3-border"><?= h($producto->moneda_id) ?></td>
